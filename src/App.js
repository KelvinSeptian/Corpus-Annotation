...
import { sendVideo, retrieveTranscript, retrieveVTT } from './API';
...
function App() {
  ...
  useEffect(() => {
    if (id !== undefined) {
      // let completed = false;
      let rec = () => { setTimeout(() => {
        retrieveTranscript(id).then(res => {
          if (res.status === 'completed') {
            setTranscript(res);
            retrieveVTT(id).then(res => setVtt(res));
            return;
          } else {
            return rec();
          }
      ...
      setTimeout(() => {
        retrieveTranscript(id).then(res => {
          if (res.status === 'completed') {
            setTranscript(res);
            retrieveVTT(id).then(res => {
              let blob = new Blob([res.data], {
                type: "text/vtt"
              })
              setVtt(URL.createObjectURL(blob))
              // setVtt(res.data)
            });
            return;
          } else {
            ...
            (
              <React.Fragment>
                <section className='left'>
                  <video className='video' width={640} height={400} controls>
                    <source src={video} type="video/mp4" />
                    <track label="English" kind="captions" srcLang="en" src={vttConvert()} default />
                  </video>
